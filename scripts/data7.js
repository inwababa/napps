	...
	function submitForm() {		
		...
		$.ajax({				
			...
			beforeSend: function(){	
				...
				$("#login_button").html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> <span> checking transaction id... </span>');
            },
            cache: false,
				...
				if(response.status==200){		
                   // $("#success").show();				
                    ...
                    setTimeout(' window.location.href = "confirm/'+response.transid+'"; ',2000);
                        ...
                        $("#error").show();					
						$("#error").html('<div class="alert alert-icon alert-warning" role="alert"><em class="fas fa-check-circle"></em><strong> Error</strong> Invalid Transaction ID Supplied</div>');
						$("#login_button").html('Pay Invoice');
